                                   ...
                                   bool profileSet)
{
    if (positionalArgs.isEmpty())
        return fail(QStringLiteral("missing command"));
        ...
        || action == QStringLiteral("mute"))
    {
        if (positionalArgs.size() != 1)
            return fail(QStringLiteral("command '%1' does not accept positional arguments").arg(action));
    ...
    if (action == QStringLiteral("refresh")) {
        if (profileSet)
            return fail(QStringLiteral("refresh does not accept --profile"));
        if (positionalArgs.size() != 1)
    ...
    if (action == QStringLiteral("get")) {
        if (profileSet)
            return fail(QStringLiteral("get does not accept --profile"));
        if (positionalArgs.size() != 2)
            return fail(QStringLiteral("usage: kv-ctl get volume|muted|active-app|apps|step|profiles"));
    ...
    if (action == QStringLiteral("set")) {
        if (profileSet)
            return fail(QStringLiteral("set does not accept --profile"));
        ...
        if (cmd.field != KvCtlCommand::Field::Volume
            && cmd.field != KvCtlCommand::Field::Muted
            && cmd.field != KvCtlCommand::Field::ActiveApp
            && cmd.field != KvCtlCommand::Field::Step)
        {
            return fail(QStringLiteral("unknown or read-only set field '%1'").arg(positionalArgs[1]));
        }
