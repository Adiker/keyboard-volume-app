...
- `struct Profile { QString id, name; QStringList apps; QString appRegex; HotkeyConfig hotkeys; QSet<Modifier> modifiers; DuckingConfig ducking; bool autoSwitch; int volMin; int volMax; QString sink; }` — `sink` is the stable PulseAudio sink **name** (empty = system default; cleared in stream-restore when the user switches back to default in Settings); `appRegex` is an optional case-insensitive regex for Follow Focus / `findProfileByApp` in addition to `apps`
...
**App identity layer** (aliases + filters). Top-level config keys plus helpers in `appmatcher.h` / `appaudiofilters.{h,cpp}`:
- `struct AppAlias { QString match, display, target; }` — `app_aliases` array; UI shows `display`, volume ops use `target` when set
- `struct AudioAppFiltersConfig` — `extra_system_binaries` / `remove_system_binaries` / `extra_skip_app_names` / `remove_skip_app_names` applied on top of built-in `SYSTEM_BINARIES` / `SKIP_APP_NAMES`
- API: `appAliases()` / `setAppAliases()`, `audioAppFilters()` / `setAudioAppFilters()`, `effectiveSystemBinaries()` / `effectiveSkipAppNames()`
- `VolumeController::setConfig(Config*)` lets `PaWorker` read identity settings on each `listApps` refresh; `AppListWidget` uses the same filters/aliases
- `appMatchCandidates()` treats `limusic-app` and `limusic` as two client names for LiMusic. This keeps an existing `limusic-app` profile working with the `limusic` binary while leaving an embedded `mpv` node separate from a standalone `mpv` profile.
...
- **`PipeWireNode` struct** — node identity plus `rawVolume`, `channelVolumes`, mute state, and helpers for the mixer-visible and effective levels. `object.serial` distinguishes a newly created node when PipeWire reuses a global id.
- **`SYSTEM_BINARIES` / `SKIP_APP_NAMES`** — built-in `QSet<QString>` defaults in `appaudiofilters.{h,cpp}`; re-exported via `pwutils.h`. User overrides come from `Config::effectiveSystemBinaries()` / `effectiveSkipAppNames()`
- **`listPipeWireClients(systemBinaries, skipAppNames)`** — snapshots the PipeWire registry through libpipewire, filters system binaries, renames skipped app names to their binary, and returns a deduplicated list. Defaults to the built-in constants. Returns empty list on connection failure or timeout.
- **Display name vs control target** — `PipeWireClient::name` is the UI label, `PipeWireClient::binary` is the owning client's control target, and `PipeWireClient::id` is the PipeWire client global id when known. Some apps expose a friendly client (`harmonoid`) but play through a technical stream node (`mpv`). In that case the UI should keep the owner binary as the control target so a separate `mpv` profile cannot claim the embedded stream.
...
- **`inspectPipeWire()` / `findPipeWireNodesForApp()` / `readPipeWireNode()`** — take read-only snapshots and read `SPA_PARAM_Props` without spawning `pw-cli`. The snapshot includes every output stream so active PA sink inputs can be joined exactly through their `object.id` property.
- **Visible-volume write contract** — PipeWire stores `SPA_PROP_volume` and `SPA_PROP_channelVolumes` as separate linear DSP gains, while PulseAudio-compatible mixers expose the cubic-root channel scale. `setPipeWireNodeVisibleVolume()` / `setPipeWireNodeVisibleChannelVolumes()` therefore write `rawVolume = 1.0` and cube each user-visible channel value. Never write a requested percentage to `SPA_PROP_volume`: WirePlumber persists that hidden multiplier even though KDE Plasma, pavucontrol, and `wpctl` continue to show the unchanged channels.
- **Exact non-volume restore** — `restorePipeWireNodeVolumeState()` is restricted to sink moves and restores the exact pre-move raw/channel representation. Startup, refresh, reconnect, focus/profile switching, routing, and discovery must remain read-only with respect to volume. A pre-existing non-unity raw multiplier is reported through `hiddenVolumeDetected` and normalized only by an explicit volume-changing action, preserving its effective gain.
- **Persistent hot-path backend** — `PaWorker` owns one `PipeWireVolumeBackend` for its lifetime. Reads, visible writes, ducking, pending replay, and routing reuse that connection instead of creating a thread loop/context/registry round-trip per hotkey event. Registry removals are tracked, a failed core is discarded and reconnected lazily on the next operation, and `PaWorker::cleanup()` closes the connection explicitly before its thread exits. The compatibility free functions remain short-lived for non-worker callers.
...
`toggleDucking(keepApp, duckVolume)` is manual per-profile Focus Audio: first call snapshots every active or idle stream except `keepApp` independently, including exact visible channel values, and lowers them to `duckVolume`; the next call restores each stream's channels. Snapshot identity survives PulseAudio reconnect and prevents two streams belonging to one app from overwriting each other. If the original representation contained a hidden raw multiplier, the snapshot folds it into the restored visible channels so normalization preserves the former effective gain. Ducking changes only volume, not mute.

4-level fallback strategy (in hot-path order):

1. **Active sink input** — enumerate through libpulse, then on pipewire-pulse use the exact `object.id` to write visible channel volume through libpipewire; native PulseAudio stays on `pa_context_set_sink_input_volume`
2. **PipeWire node** (libpipewire) — for paused/idle nodes; all matching output streams are updated
3. **Stream restore DB** (libpulse) — persists across pause/resume; on pipewire-pulse the value is also parked so the first live node is normalized to raw unity
4. **Pending watcher** — stores desired volume/mute in `PaWorker`; applies when app reconnects

**Output sink routing** (separate from volume): `listSinks()` / `setAppSink(app, sinkName)` / `clearAppSinkOverride(app)` on the PA worker thread. Cascade: snapshot each matching stream's exact raw/channel or PulseAudio channel state → move the sink-input → restore that exact state → update the stream-restore `device` field → park in `m_pendingSinks` until persistence succeeds. Routing never normalizes or otherwise changes volume, including automatic profile/focus routing. `clearAppSinkOverride` writes `device=nullptr` to stream-restore; requests are parked in `m_pendingSinkClears` while PA is down and flushed on reconnect. `PA_SUBSCRIPTION_MASK_SINK` debounces `sinksReady`. `App::activateProfile()` routes profile sinks once per `(profile, focused app)` transition (`m_lastActivatedProfileId` / `m_lastActivatedRouteApp`, cleared on `settingsChanged` and `sinksReady`). It uses `setProfileAppSink()` so `VolumeController` records dynamic `app_regex` targets; Settings includes those targets when clearing stale stream-restore overrides after a profile sink/regex change or removal. Per-profile D-Bus methods call `applyProfileSink()` before volume/mute. PipeWire `target.object` fallback is intentionally out of scope in v1.

`listApps()` returns cached data immediately and posts a background refresh that emits `appsReady(list)`. Configured `audio_app_filters` are applied consistently to active PulseAudio sink inputs and idle PipeWire clients before aliases are applied. Every refresh also performs a read-only hidden-volume scan; each affected node serial is warned once. The background watcher listens for new sink-input events and applies only volumes parked by an earlier explicit operation.

On pipewire-pulse, active sink inputs use the owning PipeWire client identity when available; PulseAudio stream fields are the fallback when owner metadata is unavailable. `listApps()` maps active streams through `client.id` where possible, so an embedded renderer is listed under its owning app and keeps the owner binary as its control target (for example display `harmonoid`, target `harmonoid`, with an embedded node named `mpv`).

`PaWorker` detects `PA_CONTEXT_FAILED` / `PA_CONTEXT_TERMINATED`, tears down the stale libpulse context, and reconnects with exponential backoff (500ms up to 30s). It identifies pipewire-pulse from `pa_server_info::server_name`; without that proof it keeps the native PulseAudio path for compatibility. PA hot paths first check `contextReady()`; if PA is down, they skip libpulse, try the PipeWire libpipewire fallback, and otherwise park pending volume/mute until the app reconnects. Reconnect itself performs no volume write except replaying a value that was already explicitly parked.
...
App/binary filter defaults (`SYSTEM_BINARIES`, `SKIP_APP_NAMES`) live in `appaudiofilters.{h,cpp}` (re-exported from `pwutils.h`) and are shared with `AppPage` via `Config::effective*` overrides.
...
`Profiles` property is `QVariantList` of `QVariantMap` entries — `{id, name, app, apps, app_regex, modifiers (QStringList "ctrl"/"shift"), hotkeys ({volume_up, volume_down, mute, show}), ducking ({enabled, volume, hotkey}), sink?}`. Hotkey values are legacy ints for `EV_KEY` bindings or maps for scroll bindings. `reloadProfiles()` rebuilds the cache from `Config` and emits `profilesChanged(QVariantList)` only when the value actually changed; wired from `TrayApp::settingsChanged` in `App`. `kv-ctl` normalizes nested `QDBusArgument` arrays/maps before rendering list-valued properties; profile output preserves its original first six tab-separated columns and appends `apps=` / `regex=` identity fields.
...
Run the isolated PipeWire regression with `-DENABLE_PIPEWIRE_INTEGRATION_TESTS=ON`, then `ctest --test-dir cpp/build -R integration_pipewire_visible_volume --output-on-failure`. Its key invariant after every application volume write is `Props:volume == 1.0`, with the intended user-visible value cubed in `Props:channelVolumes` and independently confirmed through `wpctl` and PulseAudio.
