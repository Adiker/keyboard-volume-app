...
- **Skips candidate fields that are empty** — `QString::contains("")` returns `true`, so an `AudioApp` with an empty `name` or `binary` (e.g., a sink input lacking `application.process.binary`) would otherwise match every focused window and hijack auto-switching.
