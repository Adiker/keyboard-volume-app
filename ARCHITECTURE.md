...
- `integration_pipewire_visible_volume` (opt-in) — starts private PipeWire, pipewire-pulse, WirePlumber, D-Bus, null sinks, and silent streams in a temporary XDG tree. It verifies `pw-cli enum-params ... Props`, `wpctl get-volume`, and `pactl` after relative/absolute writes, scene apply, refresh, routing, inactive stream-restore replay, pending apply, per-channel duck/restore, and a pipewire-pulse reconnect. The desktop session, live streams, and user config are inaccessible to the test processes.
