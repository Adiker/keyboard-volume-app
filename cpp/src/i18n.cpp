        ...
        {QStringLiteral("en"),
         {
             ...
             {QStringLiteral("settings.config.import"), QStringLiteral("Import config...")},
             {QStringLiteral("settings.config.export"), QStringLiteral("Export config...")},
             {QStringLiteral("settings.config.file_filter"),
              QStringLiteral("JSON files (*.json);;All files (*)")},
             {QStringLiteral("settings.config.import_title"), QStringLiteral("Import config")},
             {QStringLiteral("settings.config.export_title"), QStringLiteral("Export config")},
             {QStringLiteral("settings.config.import_confirm_title"),
              QStringLiteral("Import config")},
             {QStringLiteral("settings.config.import_confirm_msg"),
              QStringLiteral("The selected config will replace the current config and a backup "
                             "will be created first.\n\nAfter a successful import, the app must "
                             "close so it cannot overwrite the imported config with old in-memory "
                             "settings.")},
             {QStringLiteral("settings.config.restart_now"), QStringLiteral("Restart now")},
             {QStringLiteral("settings.config.exit"), QStringLiteral("Exit")},
             {QStringLiteral("settings.config.cancel"), QStringLiteral("Cancel")},
             {QStringLiteral("settings.config.export_success_title"),
              QStringLiteral("Config exported")},
             {QStringLiteral("settings.config.export_success_msg"),
              QStringLiteral("Config exported to:\n%1")},
             {QStringLiteral("settings.config.import_error_title"),
              QStringLiteral("Import failed")},
             {QStringLiteral("settings.config.import_error_msg"), QStringLiteral("%1")},
             {QStringLiteral("settings.config.export_error_title"),
              QStringLiteral("Export failed")},
             {QStringLiteral("settings.config.export_error_msg"), QStringLiteral("%1")},
             ...
             {QStringLiteral("settings.position.section"), QStringLiteral("OSD repositioning")},
             {QStringLiteral("settings.position_controls"), QStringLiteral("Allow moving the OSD")},
             {QStringLiteral("settings.position_arrows"),
              QStringLiteral("Arrow buttons (snap to screen edge)")},
             {QStringLiteral("settings.position_drag"), QStringLiteral("Mouse drag")},
             {QStringLiteral("settings.position_keyboard"),
              QStringLiteral("Keyboard shortcuts (while OSD is visible)")},
             {QStringLiteral("settings.position.snap_up"), QStringLiteral("Snap to top:")},
             {QStringLiteral("settings.position.snap_down"), QStringLiteral("Snap to bottom:")},
             {QStringLiteral("settings.position.snap_left"), QStringLiteral("Snap to left:")},
             {QStringLiteral("settings.position.snap_right"), QStringLiteral("Snap to right:")},
             {QStringLiteral("settings.position.scale_up"), QStringLiteral("Enlarge OSD:")},
             {QStringLiteral("settings.position.scale_down"), QStringLiteral("Shrink OSD:")},
             {QStringLiteral("settings.position.keyboard_hint"),
              QStringLiteral("Shortcuts work only while the OSD is visible. Leave a field empty "
                             "to disable that action. Example: arrow keys for snap, +/− for "
                             "scale.")},
             // app selector
        ...
        {QStringLiteral("pl"),
         {
             ...
             {QStringLiteral("settings.config.import"),
              QStringLiteral("Importuj konfiguracj\u0119...")},
             {QStringLiteral("settings.config.export"),
              QStringLiteral("Eksportuj konfiguracj\u0119...")},
             {QStringLiteral("settings.config.file_filter"),
              QStringLiteral("Pliki JSON (*.json);;Wszystkie pliki (*)")},
             {QStringLiteral("settings.config.import_title"),
              QStringLiteral("Importuj konfiguracj\u0119")},
             {QStringLiteral("settings.config.export_title"),
              QStringLiteral("Eksportuj konfiguracj\u0119")},
             {QStringLiteral("settings.config.import_confirm_title"),
              QStringLiteral("Importuj konfiguracj\u0119")},
             {QStringLiteral("settings.config.import_confirm_msg"),
              QStringLiteral("Wybrana konfiguracja zast\u0105pi obecn\u0105 konfiguracj\u0119, a "
                             "wcze\u015bniej zostanie utworzony backup.\n\nPo udanym imporcie "
                             "aplikacja musi si\u0119 zamkn\u0105\u0107, aby nie nadpisa\u0107 "
                             "zaimportowanej konfiguracji starymi ustawieniami z pami\u0119ci.")},
             {QStringLiteral("settings.config.restart_now"),
              QStringLiteral("Uruchom ponownie teraz")},
             {QStringLiteral("settings.config.exit"), QStringLiteral("Zamknij")},
             {QStringLiteral("settings.config.cancel"), QStringLiteral("Anuluj")},
             {QStringLiteral("settings.config.export_success_title"),
              QStringLiteral("Konfiguracja wyeksportowana")},
             {QStringLiteral("settings.config.export_success_msg"),
              QStringLiteral("Konfiguracj\u0119 wyeksportowano do:\n%1")},
             {QStringLiteral("settings.config.import_error_title"),
              QStringLiteral("Import nie powi\u00f3d\u0142 si\u0119")},
             {QStringLiteral("settings.config.import_error_msg"), QStringLiteral("%1")},
             {QStringLiteral("settings.config.export_error_title"),
              QStringLiteral("Eksport nie powi\u00f3d\u0142 si\u0119")},
             {QStringLiteral("settings.config.export_error_msg"), QStringLiteral("%1")},
             ...
             {QStringLiteral("settings.position.section"), QStringLiteral("Przenoszenie OSD")},
             {QStringLiteral("settings.position_controls"),
              QStringLiteral("Umo\u017cliw przenoszenie OSD")},
             {QStringLiteral("settings.position_arrows"),
              QStringLiteral("Strza\u0142ki (przyklej do kraw\u0119dzi ekranu)")},
             {QStringLiteral("settings.position_drag"),
              QStringLiteral("Przeci\u0105ganie mysz\u0105")},
             {QStringLiteral("settings.position_keyboard"),
              QStringLiteral("Skr\u00f3ty klawiszowe (gdy OSD widoczne)")},
             {QStringLiteral("settings.position.snap_up"),
              QStringLiteral("Przyklej do g\u00f3ry:")},
             {QStringLiteral("settings.position.snap_down"),
              QStringLiteral("Przyklej do do\u0142u:")},
             {QStringLiteral("settings.position.snap_left"), QStringLiteral("Przyklej do lewej:")},
             {QStringLiteral("settings.position.snap_right"),
              QStringLiteral("Przyklej do prawej:")},
             {QStringLiteral("settings.position.scale_up"), QStringLiteral("Powi\u0119ksz OSD:")},
             {QStringLiteral("settings.position.scale_down"), QStringLiteral("Pomniejsz OSD:")},
             {QStringLiteral("settings.position.keyboard_hint"),
              QStringLiteral(
                  "Skr\u00f3ty dzia\u0142aj\u0105 tylko gdy OSD jest widoczne. Puste pole "
                  "wy\u0142\u0105cza dan\u0105 akcj\u0119. Przyk\u0142ad: strza\u0142ki "
                  "do kraw\u0119dzi, +/\u2212 do skali.")},
             // app selector
